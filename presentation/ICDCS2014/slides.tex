...
\begin{frame}
  ...
  \item A particular HTTP request may require 35 database
    lookups.${^{1}}$
    ...
    \end{itemize}
    \myv
  \item Studies${^{2}}$ have shown that high latencies may drive customer away.  \myv
    %% \centering
    \myv
    \myv
  \item \footnotesize{[1] Nathan Farrington and Alexey Andreyev, Facebook’s Data
      Center Network Architecture.}
  \item \footnotesize{[2] Akamai Technologies Inc. and Jupiter Research Inc.
    Akamai and Jupiter Research identify `4 seconds' as the new threshold of
    acceptability for retail web page response times.
    \url{http://www.akamai.com/html/about/press/releases/20
    06/press_110606.html}.}

    %% \footnote{Akamai Technologies Inc. and Jupiter Research Inc.
    %% Akamai and Jupiter Research identify `4 seconds' as the new threshold of
    %% acceptability for retail web page response times.
...
\begin{frame}
  ...
  \begin{itemize}
  \item Ideally, clients want to specify their latency requirements regardless
    of what other clients are doing. \myv
...
\begin{frame}
  ...
  \item \textbf{Deadline Cache (DLC)}
    ...
    \item \textbf{Deadline Scheduler (DLS)}
      \begin{itemize}
        \myv
      \item Performs feedback-driven deadline-aware scheduling.
        \myv
...
\begin{frame}
  \frametitle{DLC - Properties}
  \vspace{-15 mm}
  \begin{itemize}
  \item LRU ensures high cache hit rate. \myv
  \item Adaptive eviction policy across multiple LRU queues ensures the items
    that are more likely to miss their deadlines to be kept in the cache. \myv
  \item \textbf{DLC} achieves adaptive deadline-aware caching.
  \end{itemize}
\end{frame}
