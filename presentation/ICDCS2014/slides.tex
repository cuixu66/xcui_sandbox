...
\begin{frame}
  ...
  \begin{itemize}
  \item Scheduler is responsible for controlling the access to
    data-server.
  \end{itemize}
  \begin{figure}
    \begin{center}
...
\begin{frame}
  ...
  \item A load-balancing system that directs each client to the server that is
    most likely to meets the request's deadline.
    \myv
  \item A variant of earliest deadline first scheduling that limit the impact
    of deadline violations on other requests.
    \myv
  \item A tunable admission control system that bound the percentage of
