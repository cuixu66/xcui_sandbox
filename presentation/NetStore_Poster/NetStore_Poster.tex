...
{\vspace{0mm}
\fontfamily{phv}\selectfont\textsc{{\textbf{\underline{Xu Cui}}, Michael Mior, Supervisors: Bernard Wong,
    Khuzaima Daudjee} \\ \fontfamily{ptm}\selectfont{David R. Cheriton School of Computer Science}}} % Author names and institution
...
\item The network can become a performance bottleneck.
...
\begin{center}
  \includegraphics[scale=0.190]{img/exp_setup.pdf}
  \captionof{figure}{Multi-Rooted Tree Topology}
\end{center}
\begin{itemize}
  \vspace{-1.5em}
...
\headerbox{Opportunistic Load Redistribution}{name=olr, column=1, span=2, below=netstore, bottomaligned=background}{
  ...
  %% \myv \myv \myv
  \begin{itemize}\compresslist
  \item \textbf{OLR} takes the advantage of read operation results by creating temporary replicas.
  \item \textbf{OLR} avoids complex cache eviction implementations.
  \item \textbf{OLR} avoids communication costs for cache invalidation.
  \end{itemize}
  \vfill
  \columnbreak
  \hspace{1.0em}
  \begin{center}
    ...
    \captionof{figure}{OLR Example Part I}
  \end{center}
  \vfill
  \columnbreak
  \hspace{2.5em}
  \begin{center}
    ...
    \captionof{figure}{OLR Example Part II}
  \end{center}
\end{multicols}
}
...
\headerbox{Least Bottlenecked Path}{name=lbp, column=3, span=1}{
  ...
  \myv \myv \myv
  \begin{itemize}\compresslist
  \item The NetStore controller has global view of the network.
  \item \textbf{LBP} exploits dynamic flow count information on each link to compute bandwidth allocation for each new flow.
  ...
  \begin{center}
    ...
    \captionof{figure}{LBP Example}
  \end{center}
}

\headerbox{Earliest Expected Job First}{name=eejf, column=3, span=1,  below=lbp, bottomaligned=background}{
  \textbf{EEJF} is designed to delay new flows that may traverse congested links.
  \vspace{-0.5em}
  \begin{itemize}\compresslist
  \item The NetStore controller maintains a network-aware performance model of the underlying system.
  \item \textbf{EEJF} uses this model to predict the expected completion time of a transaction.
  \item \textbf{EEJF} orders transactions using this expected completion time.
  \end{itemize}
}
...
\headerbox{Performance Evaluation}{name=performance, column=0, span=3,below=background, above=bottom}{
  \begin{multicols}{3}
    \begin{itemize}\compresslist
    \item Mininet is used in a 9-node cluster to emulate a multi-rooted tree topology with 1Gbps links.
    \item There are a total of eight Top-of-Rack switches and each is connected to eight virtual servers.
    \item A modified version of the RUBiS benchmark is deployed to evaluate NetStore.
    \item Equal-Cost Multi-Path (ECMP) routing is used
    ...
    \begin{center}
      \hspace{1.0em}
      \includegraphics[scale=0.35]{img/all_bar_with_all_bars.pdf}
  ...
  \vspace{1em}
    ...
    \item NetStore is consistently performing over 50\% better than ECMP while the network is congested.
    \item This performance improvement is achieved without sacrificing the system throughput.
    \end{itemize}
  \end{multicols}
}
\headerbox{Conclusion}{name=conclusion, column=3,below=background, above=bottom}{
  ...
  \item NetStore employs cross-layer optimizations that rely on dynamic network information to improve transaction performance.
