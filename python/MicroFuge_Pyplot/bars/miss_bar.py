...
width = 0.11      # the width of the bars: can also be len(x) sequence
...
p2 = plt.bar(ind + width * 2, mean_values,width, color='w', yerr=stderr_values, ecolor='r',
...
p4 = plt.bar(ind + width * 4,  mean_values,width, color='w',
             # yerr is for errorbar & ecolor is for errobar colour
             yerr=stderr_values, ecolor='r',
             edgecolor='black', hatch=PATTERN[4])


# IMPORTANT: commenting out the 'DLC + DLS + AC' + 'REJECTION' bar because it
# is confusing, explain this in the paper
# # this is the bottom part of the rejection
# mean_values = []
# stderr_values = []
# get_results(AC_DIR, AC_CLIENT_NUMBERS, mean_values, stderr_values, True)
# p4 = plt.bar(ind + width * 5,  mean_values,width, color='w', yerr=stderr_values,
#              edgecolor='black', hatch=PATTERN[4])

# mean_values_reject = []
# stderr_values_reject = []
# get_results(AC_DIR, AC_CLIENT_NUMBERS, mean_values_reject, stderr_values_reject, True)
# p5 = plt.bar(ind + width * 5,  mean_values_reject, width, color='w', yerr=stderr_values_reject,
#              edgecolor='black', hatch=PATTERN[5], bottom=mean_values)


mean_values = []
stderr_values = []
rw_helper.get_rw_results(CS_RW_DIR, CS_RW_CLIENT_NUMBERS, mean_values, stderr_values)
...
mean_values = []
stderr_values = []
rw_helper.get_rw_results(SH_RW_DIR, SH_RW_CLIENT_NUMBERS, mean_values, stderr_values)
